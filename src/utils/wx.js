...
export default {
  ...
  wxUserInfo1(callback) {
    ...
    if (!store.state.common.wxUserInfo||!store.state.common.wxUserInfo.headimgurl) {
  ...
  wxConfig() {
    let promise = new Promise((resolve,reject)=>{
      let url = globalConfig.rootUrl + "weixin/getConfig?param=" + encodeURIComponent(window.location.href.split('#')[0]);
      console.log(url)
      instance.get(url).then((res)=>{
        ...
        wx.config({
          ...
          jsApiList: ['onMenuShareAppMessage','hideMenuItems','showMenuItems'] // 必填，需要使用的JS接口列表，所有JS接口列表见附录2
        });
      });
    })
    return promise;
